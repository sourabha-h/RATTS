...
def process_gui(test_name,modex=""):
        ...
        if(test_name=="All"):
                ...
                for key in  objdata:
                        ...
                        doc=f"{x+" with Test Case  "+tetestcase_key}"
                ...
                for keyword in suite_keywords:
                        ...
                        for step in keyword['steps']:
                                ...
                                for key in  obj:
                                        ...
                                        testcase_key = obj[key]["testcase"]
                                        del obj[key][first_key]
                                        test=suite.tests.create(x+" Test Case for "+keyword["name"]+" "+testcase_key,doc=x+" with Test  "+testcase_key)
                                        del obj[key]["testcase"]
                                        json_objdata=json.dumps(obj[key])
        ...
        command = f"pabot --pabotlib --processes {total_key}  --outputdir Output/Metrics/Result  {modified_generated_suite_path}"
        print(command)
        ...
        # Path to directory where pabot results are stored
        base_path =  os.path.join(absolute_path,'RATTS', 'backend','Output','Metrics','Result','pabot_results')
       
        # Find all output.xml files in numbered subfolders
        output_files = glob.glob(os.path.join(base_path, "*", "output.xml"))
        print(f"output files: {output_files}")
        # Make sure we found some files
        if not output_files:
                raise FileNotFoundError("No output.xml files found in pabot_results subfolders.")

        # Path for merged output file
        merged_output = os.path.join(absolute_path,'RATTS', 'backend','Output','Metrics','Result','output.xml')
        modified_merged_output=merged_output.replace('\\', '\\\\')
        # Run rebot with merge
        cmd = ["rebot", "--output", modified_merged_output, "--merge"] + output_files
        rebres=subprocess.run(cmd)  
        print(f"rebres Return Code: {rebres.returncode}")
        print(f"rebres STDOUT: {rebres.stdout}")
        print(f"rebres STDERR: {rebres.stderr}")
