...
def display_permissions():
    ...
    try:
        ...
        # Return the result
        return {"details": permissions_result, "permission_names": updated_permission_names}
...
def last_5_reg_runs():
    try:
        ...
        debug_query = """
        SELECT COUNT(*) FROM regression_tests
        """
        cursor.execute(debug_query)
        total_count = cursor.fetchone()[0]
        print(f"Total records in regression_tests: {total_count}")

        # Check how many records have schedule_id
        debug_query2 = """
        SELECT COUNT(*) FROM regression_tests 
        ...
        print(f"Records with schedule_id: {scheduled_count}")

        # Modified main query to be more lenient
        query = """
        ...
        print(f"Found {len(rows)} rows with modified query")
        ...
        last_runs = [
            {
                ...
                "Failed": row[3],
                "system_name": row[4],
                "html": f"http://127.0.0.1:8000/Output/Metrics/Result/{row[5]}"
            }
            for row in rows
        ]
        ...
        return None
"""
def get_top_5_failed_consecutive_cases():
    try:
        cursor = get_cursor()

        # Debug query to check unit_tests table
        debug_query = ""
        SELECT COUNT(*) FROM unit_tests
        ""
        cursor.execute(debug_query)
        total_count = cursor.fetchone()[0]
        print(f"Total records in unit_tests: {total_count}")
        ...
        SELECT 
            ...
            system_name, 
            task_name, 
            COUNT(*) as failed_consecutive_runs
        FROM 
            unit_tests 
        WHERE 
            total_fail >= 1 
            AND schedule_id IS NULL  -- Changed from NOT IN to IS NULL
        GROUP BY 
            parent_system_name, system_name, task_name
        ORDER BY 
            failed_consecutive_runs DESC
        LIMIT 5
        ""

        cursor.execute(query)
        rows = cursor.fetchall()
        print(f"Found {len(rows)} failed cases")
        results = []
        
        for row in rows:
            parent_system_name = row[0]
            system_name = row[1]
            task_name = row[2]
            failed_consecutive_runs = row[3]
            ...
            WHERE 
                total_fail >= 1
                AND schedule_id IS NULL  -- Changed from NOT IN to IS NULL
                AND parent_system_name = ?
                AND system_name = ?
                AND task_name = ?
            ORDER BY 
                execution_date DESC
            ""
            cursor.execute(details_query, (parent_system_name, system_name, task_name))
            failure_details = cursor.fetchall()
            
            details = [{"timestamp": detail[0], "error_description": detail[1], "file":f"http://127.0.0.1:8000/Output/Metrics/Result/{detail[2]}"} for detail in failure_details]

            results.append({
                "system_name": parent_system_name,
                "sub_system_name": system_name,
                "template_name": task_name,
                "failed_consecutive_runs": failed_consecutive_runs,
                "details": details
            })
...
def get_top_5_failed_consecutive_cases():
    try:
        ...
        INNER JOIN (
            ...
            GROUP BY 
                parent_system_name, system_name, task_name
        ) ut2 ON ut1.parent_system_name = ut2.parent_system_name
            AND ut1.system_name = ut2.system_name
            AND ut1.task_name = ut2.task_name
            AND ut1.execution_date = ut2.latest_date
        ORDER BY 
            (SELECT COUNT(*) FROM unit_tests 
             WHERE parent_system_name = ut1.parent_system_name 
             AND system_name = ut1.system_name 
             AND task_name = ut1.task_name) DESC
        """
        ...
        for row in rows:
            ...
            # Format the URL
            file_url = f"http://127.0.0.1:8000/Output/Metrics/Result/{result_file}"
...
async def testing_execution(schedule_id, system_name, test_type, created_by, template_name, modex):
    try:
        ...
        for template in template_names:
            ...
            if system_name=="opcode":
                response=process_opcode(opcodekey, modex)
            elif system_name=="soap":
                 process_soap(opcodekey, modex)
            elif system_name=="oepsoap":
                 process_oepsoap(opcodekey, modex)
            elif system_name=="oap":
                 process_gui(opcodekey, modex)
...
def fetch_table_scheduler_details(login_username):
    ...
    try:
        ...
        SELECT 
            ...
            updated_by, updated_at, deleted_by, deleted_at, status
        FROM table_scheduler 
        WHERE created_by = ?
        ORDER BY created_at DESC
        """
        ...
        for row in rows:
            try:
                ...
                # Generate schedule dates using helper function
            ...
            status = "DELETED"if row[12] else"ACTIVE"
        ...
        return {
            "status": "success",
            "message": f"Found {len(schedules)} schedules for user {login_username}.",
            "data": schedules
        }
    ...
    for test in regression_tests:
         if test[7] != 0:   
            regression_results.append({
                ...
                'total_fail': test[4],
                'description': test[5],
                'html': f"http://127.0.0.1:8000/Output/Metrics/Result/{test[6]}",
                'schedule_id': test[7]
            })
    ...
    for test in unit_tests:
        if test[8] != 0:  # Exclude entries with schedule_id == 0
            unit_results.append({
                ...
                'task_name': test[5],
                'description': test[6],
                'html': f"http://127.0.0.1:8000/Output/Metrics/Result/{test[7]}",
                'schedule_id': test[8]
            })
